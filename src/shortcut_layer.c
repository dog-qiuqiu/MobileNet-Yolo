    ...
    ACTIVATION activation, int train)
{
    ...
    float memory;
    memory = (4*(l.out_w*l.out_h*l.out_c*2))/(1024.*1024.);

    l.bflops = l.out_w * l.out_h * l.out_c * l.n / 1000000000.;
    if (l.weights_type) l.bflops *= 2;
    fprintf(stderr, " wt = %d, wn = %d, outputs:%4d x%4d x%4d %5.3f BF %5.3f MB\n", l.weights_type, l.weights_normalizion, l.out_w, l.out_h, l.out_c, l.bflops, memory);
    return l;
}
